...
const RadarIcon: React.FC<{ spinning?: boolean }> = ({ spinning = false }) => (
    <svg viewBox="0 0 100 100" className="w-full h-full text-cyan-400" preserveAspectRatio="xMidYMid meet">
        <defs>
        ...
        <g filter="url(#cyanGlow)" stroke="currentColor" strokeWidth="1" fill="none">
            ...
            {spinning && (
                <path d="M 50 50 L 50 5 A 45 45 0 0 1 85.35 27.5 Z" fill="currentColor" opacity="0.3" className="animate-sweep" />
            )}
        </g>
    </svg>
);
...
const SignalIcon: React.FC = () => (
     <svg viewBox="0 0 100 20" className="w-full h-full text-cyan-400 opacity-10" preserveAspectRatio="none">
        <path d="M0 10 Q 12.5 0, 25 10 T 50 10 T 75 10 T 100 10" stroke="currentColor" strokeWidth="1.5" fill="none" opacity="0.8" className="text-glow-cyan"/>
    </svg>
);

const CenterpieceIcon: React.FC = () => (
    <svg viewBox="0 0 100 100" className="w-full h-full text-red-500 animate-pulse-light" preserveAspectRatio="xMidYMid meet">
...
const RedRadarIcon: React.FC = () => (
    <svg viewBox="0 0 100 100" className="w-full h-full text-red-500" preserveAspectRatio="xMidYMid meet">
...
const BreakingNewsPanel: React.FC = () => (
    <Panel className="h-full" contentClassName="p-3 flex flex-col justify-center">
        ...
        <div className="flex space-x-2 text-xs sm:text-[10px] xl:text-xs">
            <button className="flex-1 p-1 border border-red-500/50 text-red-400/80 hover:bg-red-500/20 hover:text-red-400 transition-colors">ALL SPORTS</button>
            <button className="flex-1 p-1 border border-red-500/50 text-red-400/80 hover:bg-red-500/20 hover:text-red-400 transition-colors">SPORT A</button>
        </div>
    </Panel>
);
...
const CommandPanel: React.FC = () => (
    <Panel className="h-full" contentClassName="p-3">
        ...
        <div className="space-y-2 text-xs sm:text-sm">
            ...
            <p className="text-neutral-200">{'>'} All systems are operational</p>
        </div>
    </Panel>
);
...
const MemoryPanel: React.FC = () => (
    <div className="h-full relative pt-4">
         <div className="absolute top-0 right-4 flex items-center space-x-2 z-20 text-red-400 text-glow-red text-xs sm:text-sm uppercase tracking-widest">
            <span>MEMORY</span>
            <div className="w-16 h-0.5 bg-red-500/80"></div>
        </div>
        <Panel className="h-full" contentClassName="p-3">
...
const DaemonsPanel: React.FC = () => {
    ...
    useEffect(() => {
        const timers = [
            setTimeout(() => setWidths(w => w.map((v, i) => i === 0 ? 90 : v)), 100),
            setTimeout(() => setWidths(w => w.map((v, i) => i === 1 ? 80 : v)), 200),
            setTimeout(() => setWidths(w => w.map((v, i) => i === 2 ? 95 : v)), 300),
            setTimeout(() => setWidths(w => w.map((v, i) => i === 3 ? 60 : v)), 400),
            setTimeout(() => setWidths(w => w.map((v, i) => i === 4 ? 70 : v)), 500),
        ];
            ...
            <div className="space-y-2 sm:space-y-2.5 pt-1">
                {widths.map((width, index) => (
                    <div key={index} className="w-full bg-red-900/50 h-1.5 sm:h-2">
                        <div className="bg-red-500 h-full transition-all duration-1000 ease-out" style={{width: `${width}%`}}></div>
                    </div>
                ))}
            </div>
        </Panel>
...
export default function App() {
  return (
    <main className="bg-[#0a0a0a] text-red-400 min-h-screen p-2 sm:p-4 lg:p-6 flex items-center justify-center">
        ...
        <div className="relative z-10 grid grid-cols-12 grid-rows-12 gap-4 w-full h-full">
            {/* Top Left Title */}
             <div className="col-start-1 col-span-4 row-start-1 row-span-1">
                 <Panel className="h-full" contentClassName="flex items-center justify-start p-3">
            ...
            {/* Left Column */}
            <div className="col-start-1 col-span-4 row-start-2 row-span-3"><NovaNewsPanel/></div>
            <div className="col-start-1 col-span-4 row-start-5 row-span-2"><BreakingNewsPanel /></div>
            <div className="col-start-1 col-span-4 row-start-7 row-span-2"><CommandPanel/></div>
            <div className="col-start-2 col-span-2 row-start-9 row-span-3">
                <Panel className="h-full" contentClassName="p-3"><RedRadarIcon /></Panel>
            </div>
            ...
            <div className="col-start-5 col-span-4 row-start-9 row-span-2">
                <Panel className="h-full" contentClassName="flex items-center justify-center p-3">
                    <SignalIcon />
                </Panel>
            </div>
            
            {/* Right Column */}
            <div className="col-start-9 col-span-4 row-start-2 row-span-3"><MemoryPanel /></div>
            <div className="col-start-9 col-span-4 row-start-5 row-span-3"><DaemonsPanel /></div>
            <div className="col-start-9 col-span-4 row-start-8 row-span-4">
                <Panel className="h-full" contentClassName="p-3"><Globe/></Panel>
